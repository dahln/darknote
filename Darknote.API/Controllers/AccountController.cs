...
namespace Darknote.API.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest model)
        {
            ...
            //Example: Only allow users who's email address ends with a '@darknoteapp.com'
            ...
            if (result.Succeeded)
            {
                ...
                Darknote.Database.Note tutorialNote = new Darknote.Database.Note()
                {
                    OwnerId = newUser.Id,
                ...
                Darknote.Database.Note tutorialList = new Darknote.Database.Note()
                {
                    OwnerId = newUser.Id,
                    Title = "Shopping List",
                    ListEnabled = true,
        ...
        async public Task<IActionResult> DeleteAccount()
        {
            ...
            var notesToDelete = _db.Notes.Where(x => x.OwnerId == userId);
            
            var idsOfOwnedNotes = notesToDelete.Select(x => x.Id).Distinct().ToList();
            var noteListItemsToDelete = _db.NoteListItems.Where(x => idsOfOwnedNotes.Contains(x.Id));
            var shares = _db.NoteAuthorizedUsers.Where(x => idsOfOwnedNotes.Contains(x.NoteId));

            _db.NoteAuthorizedUsers.RemoveRange(shares);
            _db.NoteListItems.RemoveRange(noteListItemsToDelete);
